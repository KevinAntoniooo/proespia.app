...
def historial_pdf(cliente_id, user_id):
    ...
    if 'bitacoras' in partes:
        ...
        if bitacoras:
            ...
            pdf.cell(35, 7, 'TIPO VISITA', 1, 0, 'C', True)
            pdf.cell(85, 7, 'DESCRIPCION', 1, 0, 'C', True)
            pdf.cell(35, 7, 'TECNICO', 1, 1, 'C', True)
            pdf.set_font('Arial', '', 7)
            for b in bitacoras:
                pdf.cell(35, 6, enc(b.fecha.strftime('%d/%m/%Y') if b.fecha else ''), 1, 0, 'C')
                pdf.cell(35, 6, enc(b.tipo_visita), 1, 0, 'C')
                pdf.cell(85, 6, enc((b.descripcion or '')[:150]), 1, 0, 'C')
                pdf.cell(35, 6, enc(b.rel_usuario.nombre if b.rel_usuario else '---'), 1, 1, 'C')
        else:
            pdf.set_font('Arial', '', 9)
            pdf.set_text_color(150, 150, 150)
