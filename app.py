...
def historial_pdf(cliente_id, user_id):
    ...
    if 'bitacoras' in partes:
        ...
        if bitacoras:
            ...
            pdf.cell(50, 7, 'TIPO VISITA', 1, 0, 'C', True)
            pdf.cell(100, 7, 'DESCRIPCION', 1, 1, 'C', True)
            pdf.set_font('Arial', '', 7)
            for b in bitacoras:
                pdf.cell(40, 6, enc(b.fecha.strftime('%d/%m/%Y') if b.fecha else ''), 1, 0, 'C')
                pdf.cell(50, 6, enc(b.tipo_visita), 1, 0, 'C')
                pdf.cell(100, 6, enc((b.descripcion or '')[:200]), 1, 1, 'C')
        else:
            pdf.set_font('Arial', '', 9)
            pdf.set_text_color(150, 150, 150)
    ...
    if 'visitas' in partes:
        ...
        if visitas:
            ...
            pdf.cell(70, 7, 'TIPO TRABAJO', 1, 0, 'C', True)
            pdf.cell(75, 7, 'ESTADO', 1, 1, 'C', True)
            pdf.set_font('Arial', '', 8)
            for v in visitas:
                pdf.cell(45, 6, enc(v.fecha_programada.strftime('%d/%m/%Y %H:%M') if v.fecha_programada else ''), 1, 0, 'C')
                pdf.cell(70, 6, enc(v.tipo_trabajo), 1, 0, 'C')
                pdf.cell(75, 6, enc(v.estado), 1, 1, 'C')
        else:
            pdf.set_font('Arial', '', 9)
            pdf.set_text_color(150, 150, 150)
